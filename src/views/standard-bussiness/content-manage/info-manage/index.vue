<template>
  <div class="container">
    ...
    <el-button type="primary" @click="$router.push('/standard-bussiness-data/info-manage-add')">新增</el-button>
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <span>资讯列表</span>
      </div>
      <div class="table-body">
        <el-table :data="articalList" style="width: 100%" v-loading="loading" :show-header="false">
          <el-table-column prop="date" label="状态">
            <template slot-scope="scope">
              <div>
                ...
                <p class="color-green" v-if="scope.row.status === 'draft'">{{articalState(scope.row.status)}}</p>
                <p class="color-red" v-if="scope.row.status === 'forbid'">{{articalState(scope.row.status)}}</p>
              </div>
            </template>
          </el-table-column>
          <el-table-column label="图片">
            <template slot-scope="scope">
              <div class="coverPath">
                ...
                <div>
                  <p>
                    <span class="recommend" v-if="scope.row.recommend">推荐</span>
          ...
          <el-table-column label="作者">
            <template slot-scope="scope">
              <div>
                {{scope.row.expert?scope.row.expert.name:''}}
              </div>
            </template>
          </el-table-column>
          <el-table-column label="操作" align="center">
            <template slot-scope="scope">
              <div>
                <el-button type="primary" size="mini" @click="$router.push('/standard-bussiness-data/info-manage-edit/' + scope.row.id)" v-if="scope.row.status !== 'forbid'">编辑</el-button>
                <el-button type="primary" size="mini" @click="handleShow(scope.row.id)" v-if="scope.row.status === 'published'">查看</el-button>
                <el-button type="danger" size="mini" @click="cancelRecommend(scope.row.id)" v-if="scope.row.recommend && scope.row.status === 'published'">取消推荐</el-button>
                <el-button type="success" size="mini" @click="submitRecommend(scope.row.id)" v-if="!scope.row.recommend && scope.row.status === 'published'">推荐</el-button>
              ...
              <div>
                <el-button type="info" size="mini" disabled v-if="scope.row.status === 'published' && scope.row.pushed">已推送</el-button>
                <el-button type="warning" size="mini" @click="pushInfo(scope.row.id)" v-if="scope.row.status === 'published' && (!scope.row.pushed)">推送</el-button>
              </div>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  data() {
    return {
      ...
      loading: true,
      recommend: null, //是否推荐
      recommendList: [{ value: true, label: "是" }, { value: false, label: "否" }]
    };
  },
  created(){
  ...
  methods: {
    ...
    dataList: function(json) {
      ...
      this.$http
        .get("/news", {
          params: {
            type: "info", //文章类型, 非必选, mingyi对应名医说, news对应资讯, selection对应精选
