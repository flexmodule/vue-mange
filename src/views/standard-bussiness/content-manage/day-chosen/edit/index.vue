<template>
  <div class="container">
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>编辑精选文章</el-breadcrumb-item>
    </el-breadcrumb>
    <el-card class="box-card">
      <div class="info picture">
        <div>
          <p>标题：</p>
          <div>
            <el-input v-model="input" placeholder="请在这里输入标题"></el-input>
          </div>
        </div>
      </div>
      <div class="ue">
...
export default {
  ...
  mounted() {
    this.articalId = this.$route.params.articalId;
    this.getData();
  },
  methods: {
    getData() {
      ...
      this.$http
        ...
        .then(res => {
          ...
          this.showLogoImg = res.coverPath;
          this.logoImg = res.coverId;
          this.input = res.title;
          loading.close();
        })
        .catch(err => {
          loading.close();
    ...
    submitData(status, state) {
      ...
      this.$http
        .put(`/news/${this.articalId}`, {
          type: "selection",
          title: this.input, //文章标题
