<template>
  <div class="container">
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item :to="{ path: '/standard-bussiness-data/content-manage' }">名医说管理</el-breadcrumb-item>
      <el-breadcrumb-item>新增名医说</el-breadcrumb-item>
    </el-breadcrumb>
    <el-card class="box-card" v-loading="loading">
      <div class="info picture">
        <div>
          <p>标题：</p>
          <div>
            <el-input v-model="title" placeholder="请在这里输入标题" :maxlength="textLength"></el-input>
          </div>
        </div>
        <div>
          <p>作者：</p>
          <div class="authors">
            <el-select v-model="value6" placeholder="请选择作者">
              <el-option v-for="item in experts" :key="item.id" :label="item.realName" :value="JSON.stringify(item)" class="author">
                <div class="author">
                  <div class="img">
                    <img :src="item.avatarPath" alt="" width="50px" height="50px" v-if="item.avatarPath != null">
                    <img src="../../../../../static/images/expert_avatar.png" alt="" width="50px" height="50px" v-if="item.avatarPath === null">
                  </div>
                  <div>
                    <p>{{ item.realName }}</p>
                    <p>
      ...
      <div class="ue">
        <p>正文：</p>
        <div>
          <editor ref="myTextEditor" v-model="content">
          </editor>
        </div>
      </div>
      <div class="info picture">
  ...
  methods: {
    ...
    submitData(status, state) {
      if (
        ...
        this.loading = true;
        if (this.value6) {
          this.authorId = JSON.parse(this.value6).id;
          this.author = JSON.parse(this.value6).realName;
        }
        if (this.id) {
          this.$http
            .put("/news/" + this.id, {
    ...
    expertList() {
      this.$http
        ...
        .then(res => {
          if (res.data.rc === 0) {
            this.experts = res.data.data.resultList;
          } else {
            this.$message.error(res.data.msg);
          }
        })
